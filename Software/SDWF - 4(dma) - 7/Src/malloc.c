...
/*==内存池(32字节对齐)==*/
//stm32为32位cpu，一个机器字长为32位(4个字节)，所以stm32以<=4个字节为单位读写数据
//__align(32)关键字内存对齐确保数组地址和大小以32的倍数存在
__align(32) uint8_t mem1base[MEM1_MAX_SIZE];																				//内部SRAM内存池
__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(Bank1_SRAM3_ADDR)));	//外部SRAM内存池 
...
uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(Bank1_SRAM3_ADDR+MEM2_MAX_SIZE)));	//外部SRAM内存池MAP
